...
eps,gmB,gc,gx1 = (29.96879778, -1.30,  0.45,  0.06703621) # (29.96879778, -1.34334963,  0.45895811,  0.06703621)
selection_param=(gc, gx1, gmB, eps)
print("sel params", selection_param)
...
true_alpha = 0.154 # negative in mB eq
true_beta = 2.2
true_alpha_c = 0.008 # negative in mB eq
true_RB = 4.0
...
true_x1_mean = 0.0 #-0.432
true_c_mean = 0.0 #-0.061 # maybe set to 0 ?
Rx = 1.0
Rc = 0.1
...
def get_magnitude(mu, M, alpha, x1, beta, colour, alpha_c, R_B, E):

    # first get colour equation
    #cs_int = colour - (alpha_c*x1)

    # get apparent colour with dust
    #cs_app = cs_int + R_B*E

    # then get intrinsic magnitude
    Ms_int = M - (alpha * x1) + (beta*colour)

    # then finally add all together to get mB
    return mu + Ms_int + (R_B - beta)*E #(cs_app - cs_int)
...
#     # then get intrinsic magnitude
#     Ms_int = M - (alpha * x1) + (beta*cs_int)

#     # then finally add all together to get mB
#     return mu + Ms_int + (cs_app - cs_int)
...
def generate_sn1a():
    ...
    while(z<0):
        ...
        q = ss.powerlaw.rvs(1.5, loc=1.0, scale=3.0, size=()) # x = z+1 from Dilday et al
        z = q-1
        #z = np.random.normal(loc=Zcmb_mean, scale = Zcmb_std)
    ...
    #Step 3.1 Draw dust coeff
    E_i = np.random.exponential(scale=true_tau)


    #Step 4, compute exact mb* from distance modulus
    #mb = get_magnitude(mu_i,M_i, true_alpha, x1_i, true_beta, c_i, true_RB, E_i)

    mb = get_magnitude(mu_i, M_i, true_alpha, x1_i, true_beta, c_i, true_alpha_c, true_RB, E_i)
    ...
    observed_dx1 = np.random.normal(loc=dx1_mean, scale = dx1_std) #ss.foldnorm.rvs(0.0, loc=dx1_mean, scale=dx1_std) #np.abs(np.random.normal(loc=dx1_mean, scale = dx1_std))
...
plt.savefig('bayesn_simulatedsne.png')
...
plt.savefig('bayesn_simuated_selection_fn.png')
#plt.show()
